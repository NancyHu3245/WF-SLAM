...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const cv::Mat &maskLeft, const cv::Mat &maskRight,const double &timestamp)
{
    ...
    //mycode
    // mCurrentFrame = Frame(mImGray,imGrayRight,imMaskLeft,imMaskRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    // cv::Mat imMaskg=cv::Mat::zeros(480,640,CV_32F); 
    // mGeometry.DetectMoving(mCurrentFrame,imDepth,imMask,mImGray,imMaskg);
    // mCurrentFrame.CalculEverything(mImRGB,mImGray,mImDepth,imMask,imMaskg);
    //end
...
//dynaslam
    ...
    // LightTrack();//初步估计mCurrentFram的位姿，不加局部优化，位姿MVG部分需要用

    // imRGBOut = _imRGB;

    // if (!mCurrentFrame.mTcw.empty())
    // {
    //     mGeometry.GeometricModelCorrection(mCurrentFrame,imDepth,imMask);//imMask融合为rcnn和MVG综合输出
    // }
...
//my
    ...
    mCurrentFrame = Frame(mImGray,imDepth,imMask,timestamp,_imRGB,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    //mCurrentFrame.mpReferenceKF=mpReferenceKF;
    cv::Mat imMaskg=cv::Mat::zeros(480,640,CV_32F);

#ifdef COMPILEDWITHC11
        std::chrono::steady_clock::time_point tdy_s = std::chrono::steady_clock::now();
#else
        std::chrono::monotonic_clock::time_point tdy_s = std::chrono::monotonic_clock::now();
#endif
    mGeometry.DetectMoving(mCurrentFrame,imDepth,imMask,mImGray,imMaskg);    
    mCurrentFrame.CalculEverything(mImRGB,mImGray,mImDepth,imMask,imMaskg);

#ifdef COMPILEDWITHC11
        std::chrono::steady_clock::time_point tdy_e = std::chrono::steady_clock::now();
#else
        std::chrono::monotonic_clock::time_point tdy_e = std::chrono::monotonic_clock::now();
#endif
    double tdy= std::chrono::duration_cast<std::chrono::duration<double> >(tdy_e - tdy_s).count();
    cout<<"detect dynamic object time is "<<tdy<<endl;
    //mCurrentFrame = Frame(mImGray,imDepth,imMask,imRGBOut,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    KeyFrame* pKF = Track();
    imRGBOut = _imRGB;
//end
    ...
    if(pKF!=nullptr)
    {
        mpPointCloudMapping->insertKeyFrame2( pKF, mCurrentFrame.mImRGB,imDepth,idk);
    }    
    return mCurrentFrame.mTcw.clone();
}
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        mpLocalMapper->InsertKeyFrame(pKFini);//插入关键帧到局部建图
        //mpPointCloudMapping->insertKeyFrame2( pKFini, this->mImRGB, this->mImDepth,idk);
        mLastFrame = Frame(mCurrentFrame);
...
void Tracking::UpdateLastFrame()
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        if(bCreateNew)
        {
            ...
            MapPoint* pNewMP = new MapPoint(x3D,mpMap,&mLastFrame,i);//普通帧的地图点
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // Optimize frame pose with all matches
    //Optimizer::PoseWeightOptimization(&mCurrentFrame);
#ifdef COMPILEDWITHC11
...
#endif
    double toptmize = std::chrono::duration_cast<std::chrono::duration<double> >(t2 - t1).count();
    cout<<"optimize time is "<<toptmize<<endl;
...
KeyFrame* Tracking::CreateNewKeyFrame()
{
    if(!mpLocalMapper->SetNotStop(true))//表示我要设置不停止失败，失败的原因是此时已经停止（mbStopped＝true）
        return nullptr;
    ...
    mpLocalMapper->SetNotStop(false);//将局部建图设为停止

    //mpPointCloudMapping->insertKeyFrame2( pKF, mCurrentFrame.mImRGB,mCurrentFrame.mImDepth,idk);
    mnLastKeyFrameId = mCurrentFrame.mnId;
    mpLastKeyFrame = pKF;
    return pKF;
}
