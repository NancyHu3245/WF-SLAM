     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), mIsKeyFrame(false), mImDepth(imDepth), mImRGB(imRGB)
{
    ...
    cv::Mat kernel = getStructuringElement(cv::MORPH_ELLIPSE,
                                        cv::Size( 2*dilation_size + 1, 2*dilation_size+1 ),
                                        cv::Point( dilation_size, dilation_size ) );
    cv::erode(imMask, Mask_dil, kernel);//将mask图腐蚀
...
//Frame for My code
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imMask,
             const double &timeStamp, const cv::Mat &imRGB, ORBextractor* extractor, ORBVocabulary* voc,
             cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
    :mImMask(imMask), mpORBvocabulary(voc), mpORBextractorLeft(extractor), mpORBextractorRight(static_cast<ORBextractor*>(NULL)), mImGray(imGray),
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), mIsKeyFrame(false), mImDepth(imDepth),mImRGB(imRGB)
{
    
    mmKeyWeight.resize(480);
    for(int i=0;i<480;i++)
    {
        mmKeyWeight[i].resize(640);
        for(int j=0;j<640;j++)
            mmKeyWeight[i][j]=(1.0);
    }
...
void Frame::CalculEverything( cv::Mat &imRGB, const cv::Mat &imGray,const cv::Mat &imDepth,const cv::Mat &imS,const cv::Mat &imsg){
   
    // ORB extraction
    ExtractORB(0,imGray);
    // Delete those ORB points that fall in Mask borders (Included by Berta)
    cv::Mat Mask_dil = imS.clone();
    cv::Mat Mask_kg = imsg.clone();
    int dilation_size = 15;
    cv::Mat kernel = getStructuringElement(cv::MORPH_ELLIPSE,
                                        cv::Size( 2*dilation_size + 1, 2*dilation_size+1 ),
                                        cv::Point( dilation_size, dilation_size ) );
    cv::erode(imS, Mask_dil, kernel);//将mask图腐蚀
    ...
    if(num<=(480*640)*0.7)
    {
        ...
        {
            int val = (int)Mask_dil.at<uchar>(mvKeys[i].pt.y,mvKeys[i].pt.x);
            weight=0.5*val+0.5*(double)Mask_kg.at<uchar>(mvKeys[i].pt.y,mvKeys[i].pt.x);
            mmKeyWeight[mvKeys[i].pt.y][mvKeys[i].pt.x]=weight;
            if (val == 1)
            {
                _mvKeys.push_back(mvKeys[i]);
                _mDescriptors.push_back(mDescriptors.row(i));
                //weight=0.5*(double)Mask_kg.at<uchar>(mvKeys[i].pt.y,mvKeys[i].pt.x);
                //mmKeyWeight[mvKeys[i].pt.y][mvKeys[i].pt.x]=weight;
            }   
        }
        mvKeys = _mvKeys;
