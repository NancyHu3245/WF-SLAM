...
//Add
void Geometry::DetectMoving(const ORB_SLAM2::Frame &currentFrame,
                                        cv::Mat &imDepth, cv::Mat &mask,cv::Mat &imGray,cv::Mat &maskg)
{
    ...
    cv::erode(mask, Mask_dil, kernel);//将mask图腐蚀

    // Calculate the dynamic abnormal points and output the T matrix
   if(imGrayPre.data)
    { 
        //cv::Mat mkgray=currentFrame.mpReferenceKF->mImGray;
        //cv::Mat maskg = ProcessMovingObject(imGray,mkgray,Mask_dil);
        maskg = ProcessMovingObjectwithoutkeyfram(imGray,Mask_dil);
        //mask=maskg;
        //CombineMasksg(maskg,mask);
...
cv::Mat Geometry::ProcessMovingObject(const cv::Mat &imgray,const cv::Mat &mkgray,const cv::Mat Mask_dil)
{
    // Clear the previous data

    vector<double> dd;
    vector<double> ddk;
    std::vector<cv::Point2f> prepoint,keypoint;
    std::vector<cv::Point2f> curpoint;
    cv::Mat maskG = cv::Mat::zeros(480,640,CV_32F);

    cv::goodFeaturesToTrack(imgray,curpoint, 1000, 0.01, 8, cv::Mat(), 3, true, 0.04);   
    cv::cornerSubPix(imgray, curpoint, cv::Size(10, 10), cv::Size(-1, -1), cv::TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 20, 0.03));

    dd=caculated(imGrayPre,imgray,Mask_dil,prepoint,curpoint);//计算当前帧到上一帧的极线距离
    ddk=caculated(mkgray,imgray,Mask_dil,keypoint,curpoint);//当前帧到上一关键帧的极线距离
    double sd=0;
    if(!curpoint.empty())
    {
        for(int i=0;i<dd.size() && i<ddk.size();i++)
        {
            sd=arfa*dd[i]+(1-arfa)*ddk[i];
            if(sd >limit_dis_epi )
            {
                maskG.at<uchar>(curpoint[i].y,curpoint[i].x) = sd;//总的极线距离大于阈值 
            }        
        }
    }
...
vector<double> Geometry::caculated(const cv::Mat &imgpre,const cv::Mat &mcur,const cv::Mat Mask_dil,
                std::vector<cv::Point2f> &prepoint,std::vector<cv::Point2f> &nextpoint )
{
    dd.clear();
    prepoint.clear();
    F_prepoint.clear();
    ...
    cv::calcOpticalFlowPyrLK(mcur,imgpre, nextpoint, prepoint, state, err, cv::Size(22, 22), 5, cv::TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 20, 0.01));

	for (int i = 0; i < state.size(); i++)
    {
        if(state[i] != 0)//对应位置有光流
        {
            int dx[10] = { -1, 0, 1, -1, 0, 1, -1, 0, 1 };
            int dy[10] = { -1, -1, -1, 0, 0, 0, 1, 1, 1 };
            int val = (int)Mask_dil.at<uchar>(nextpoint[i].y,nextpoint[i].x);
            //判断FAST角点处的语义分类是否为静态，如果为动态则选择不用该特征点
            if (val !=1)
            {
                state[i] = 0;
                continue;
            }
            int x1 = nextpoint[i].x, y1 = nextpoint[i].y;
            int x2 = prepoint[i].x, y2 = prepoint[i].y;
            if ((x1 < limit_edge_corner || x1 >= mcur.cols - limit_edge_corner || x2 < limit_edge_corner || x2 >= imgpre.cols - limit_edge_corner
            || y1 < limit_edge_corner || y1 >= mcur.rows - limit_edge_corner || y2 < limit_edge_corner || y2 >= imgpre.rows - limit_edge_corner))
            {
                state[i] = 0;//去掉太靠近边缘的匹配点
                continue;
            }
            double sum_check = 0;
            for (int j = 0; j < 9; j++)
                sum_check += abs(imgpre.at<uchar>(y1 + dy[j], x1 + dx[j]) - mcur.at<uchar>(y2 + dy[j], x2 + dx[j]));
            if (sum_check > limit_of_check) state[i] = 0;//去掉图像块差异过大的匹配点
            if (state[i])
            {
    ...
    //cv::Mat maskG = cv::Mat::zeros(480,640,CV_32FC1);  
    if(F_prepoint.size()>=4 && F_prepoint.size()>= 4)
    {
        // F-Matrix
        cv::Mat mask = cv::Mat(cv::Size(1, 300), CV_8UC1);
        cv::Mat F = cv::findFundamentalMat(F_nextpoint,F_prepoint,mask, cv::FM_RANSAC, 0.1, 0.99);//ransac计算基础矩阵

        for (int i = 0; i < nextpoint.size(); i++)
        {//对当前帧提取的所有orb关键点，计算极限距离．
            dd.push_back(0);
            //if (state[i] != 0)
            //{
                double A = F.at<double>(0, 0)*nextpoint[i].x + F.at<double>(0, 1)*nextpoint[i].y + F.at<double>(0, 2);
                double B = F.at<double>(1, 0)*nextpoint[i].x + F.at<double>(1, 1)*nextpoint[i].y + F.at<double>(1, 2);
                double C = F.at<double>(2, 0)*nextpoint[i].x + F.at<double>(2, 1)*nextpoint[i].y + F.at<double>(2, 2);
                dd[i] = fabs(A*prepoint[i].x + B*prepoint[i].y + C) / sqrt(A*A + B*B);    
            //}
        }
    }
    return dd;
...
cv::Mat Geometry::ProcessMovingObjectwithoutkeyfram(const cv::Mat &imgray,const cv::Mat Mask_dil){

    std::vector<cv::Point2f> prepoint;
    std::vector<cv::Point2f> nextpoint;
    cv::Mat maskG = cv::Mat::zeros(480,640,CV_32F);
    F_prepoint.clear();
	F_nextpoint.clear();
    state.clear();
    ...
    cv::goodFeaturesToTrack(imgray,nextpoint, 1000, 0.01, 8, cv::Mat(), 3, true, 0.04);   
    cv::cornerSubPix(imgray, nextpoint, cv::Size(10, 10), cv::Size(-1, -1), cv::TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 20, 0.03));    
    cv::calcOpticalFlowPyrLK(imgray,imGrayPre, nextpoint, prepoint, state, err, cv::Size(22, 22), 5, cv::TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 20, 0.01));
    ...
    if(F_prepoint.size()>=4 && F_prepoint.size()>= 4)
    {
        ...
        {//对当前帧提取的所有orb关键点，计算极限距离．
            if (state[i] != 0)
            {
                ...
                double B = F.at<double>(1, 0)*nextpoint[i].x + F.at<double>(1, 1)*nextpoint[i].y + F.at<double>(1, 2);
                double C = F.at<double>(2, 0)*nextpoint[i].x + F.at<double>(2, 1)*nextpoint[i].y + F.at<double>(2, 2);
                d = fabs(A*prepoint[i].x + B*prepoint[i].y + C) / sqrt(A*A + B*B);    
                if(d >limit_dis_epi)
                maskG.at<uchar>(nextpoint[i].y,nextpoint[i].x) = 1;//极线距离大于阈值 
            }
        }
    }
...
void Geometry::CombineMasksg(const cv::Mat maskg, cv::Mat &mask)
{
   cv::Mat _maskr=cv::Mat::ones(mask.size(),mask.type());
   cv::Mat maskr=_maskr-mask;
   mask=maskg+maskr;
}
...
void Geometry::FillRGBD(const ORB_SLAM2::Frame &currentFrame,cv::Mat &mask,cv::Mat &imGray,cv::Mat &imDepth,cv::Mat &imRGB){
    ...
    imGrayAccumulator.convertTo(imGrayAccumulator,CV_32F);
    cv::Mat bgr[3];
    cv::split(imRGB,bgr);//将图像imRGB分割为３个单通道序列bgr
