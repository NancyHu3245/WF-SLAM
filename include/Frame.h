...
public:
    ...
    // Constructor for my RGB-D code.    
    Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imMask, const double &timeStamp, const cv::Mat &imRGB,ORBextractor* extractor, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth);

    // Constructor for RGB-D cameras-5.
    ...
    // Constructor for RGB-D cameras-7.   
    ...
    //Add
    void CalculEverything(cv::Mat &imRGB, const cv::Mat &imGray,const cv::Mat &imDepth,const cv::Mat &imS,const cv::Mat &imsg);
...
public:
    ...
    //add-2020-06009
    // std::vector<std::vector<int>> mmKeyWeight;
    std::vector<std::vector<double>> mmKeyWeight;
    //std::unordered_map<cv::KeyPoint,double> mmKeyWeight;
