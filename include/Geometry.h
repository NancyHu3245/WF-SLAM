...
public:
    ...
    //Add
    void DetectMoving(const ORB_SLAM2::Frame &currentFrame,
                                        cv::Mat &imDepth, cv::Mat &mask,cv::Mat &imGray,cv::Mat &maskg);
    cv::Mat ProcessMovingObjectwithoutkeyfram(const cv::Mat &imgray,const cv::Mat Mask_dil);
    cv::Mat ProcessMovingObject(const cv::Mat &imgray,const cv::Mat &mkgray,const cv::Mat Mask_dil);
    vector<double> caculated(const cv::Mat &imgpre,const cv::Mat &mcur,const cv::Mat Mask_dil,
                std::vector<cv::Point2f> &prepoint,std::vector<cv::Point2f> &nextpoint );
    void CombineMasksg(const cv::Mat maskg, cv::Mat &mask);
