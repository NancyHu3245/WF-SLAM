...
int main(int argc, char **argv)
{
    ...
    WF_SLAM::SegmentDynObject *MaskNet;
    if (argc==6 || argc==7)
    {
        cout << "Loading Mask R-CNN. This could take a while..." << endl;
        MaskNet = new WF_SLAM::SegmentDynObject();
    ...
    cv::Mat kernel = getStructuringElement(cv::MORPH_ELLIPSE,//椭圆形核
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        //imRGB = cv::imread(string(argv[3])+"/"+vstrImageFilenamesRGB[ni],1);
        //imD = cv::imread(string(argv[3])+"/"+vstrImageFilenamesD[ni],1);
        imRGB.cv::Mat::convertTo(imRGB,CV_8U);      
        ...
        if (argc == 6 || argc == 7)
        {
            cv::Mat maskRCNN = cv::Mat::zeros(480,640,CV_8U);
            ...
            cv::dilate(maskRCNN,maskRCNNdil, kernel);//将mask-rcnn的输出结果膨胀（卷积取最大值），得到maskRCNNdil

            char filename[256];
            sprintf(filename, "mask/mask3.yaml");
            ...
            for (int i = 0; i < 640 ; i++)
            {
                for(int j = 0; j< 480; j++)
                {
                    fprintf(fp, "%d,", (int)maskRCNNdil.at<uchar>(j,i));
                }       
            }
            fclose(fp);

            //maskRCNNdil.cv::Mat::convertTo(maskRCNNdil,CV_32F);
            mask = mask - maskRCNNdil;//1矩阵－maskRCNNdil；
        }
#ifdef COMPILEDWITHC11
        std::chrono::steady_clock::time_point t_mask = std::chrono::steady_clock::now();
#else
        std::chrono::monotonic_clock::time_point t_mask = std::chrono::monotonic_clock::now();
#endif
        double tmask= std::chrono::duration_cast<std::chrono::duration<double> >(t_mask - t1).count();
        cout<<"get mask time is:"<< tmask<< endl;
    ...
    // SLAM.SaveTrajectoryTUM("CameraTrajectory_WJOS-SLAM-Z_16.txt");
    // SLAM.SaveKeyFrameTrajectoryTUM("KeyFrameTrajectory_WJOS-SLAM-Z_16.txt");
    SLAM.SaveTrajectoryTUM("CameraTrajectory_WF-SLAM-w_xyz.txt");
    SLAM.SaveKeyFrameTrajectoryTUM("KeyFrameTrajectory_WF-SLAM-w_xyz.txt");
